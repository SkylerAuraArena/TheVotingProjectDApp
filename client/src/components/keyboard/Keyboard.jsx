...
const Keyboard = () => {
  ...
  const adminModeControlBtnArray = mainContextState.keyboardBtnTxt.mainOptions.admin.map((elt, index) => {
    const workflowEventArrayLegth = mainContextState.workflowStatusChangedPastEvents.length;
    if(elt.func.params) {
      ...
      return mainContextState.currentWorkflowStatus === workflowStatus.registeringVoters && workflowEventArrayLegth === 0 ? <Button key={index} title={elt.txt} color={elt.css} func={newFunc}/> : null
    } else {
      switch (elt.func.name) {
        case solidityFunctionsList.admin.startProposalsRegistering:
          return mainContextState.currentWorkflowStatus === workflowStatus.registeringVoters && workflowEventArrayLegth === 0 ? <Button key={index} title={elt.txt} color={elt.css} func={elt.func} args={true}/> : null;
        case solidityFunctionsList.admin.endProposalsRegistering:
          return mainContextState.currentWorkflowStatus === workflowStatus.proposalsRegistrationStarted || mainContextState.workflowStatusChangedPastEvents[workflowEventArrayLegth - 1]?.lastCurrentStatus === "1" ? <Button key={index} title={elt.txt} color={elt.css} func={elt.func} args={true}/> : null;
        case solidityFunctionsList.admin.startVotingSession:
          return mainContextState.currentWorkflowStatus === workflowStatus.proposalsRegistrationEnded || mainContextState.workflowStatusChangedPastEvents[workflowEventArrayLegth - 1]?.lastCurrentStatus === "2" ? <Button key={index} title={elt.txt} color={elt.css} func={elt.func} args={true}/> : null;
        case solidityFunctionsList.admin.endVotingSession:
          return mainContextState.currentWorkflowStatus === workflowStatus.votingSessionStarted || mainContextState.workflowStatusChangedPastEvents[workflowEventArrayLegth - 1]?.lastCurrentStatus === "3" ? <Button key={index} title={elt.txt} color={elt.css} func={elt.func} args={true}/> : null;
      ...
      switch (elt.func.name) {
        case solidityFunctionsList.voter.getVotersVotes:
          if (mainContextState.currentWorkflowStatus === workflowStatus.votingSessionStarted || 
              ...
              mainContextState.workflowStatusChangedPastEvents[workflowEventArrayLegth - 1]?.lastCurrentStatus >= "3") {
              return <Button key={index} title={elt.txt} color={elt.css} func={newFunc}/>;
          } else {
            return null;
          }
        case solidityFunctionsList.voter.getOneProposal:
          return mainContextState.currentWorkflowStatus !== workflowStatus.registeringVoters || mainContextState.workflowStatusChangedPastEvents[workflowEventArrayLegth - 1]?.lastCurrentStatus > "0" ? <Button key={index} title={elt.txt} color={elt.css} func={newFunc}/> : null;
        case solidityFunctionsList.voter.addProposal:
          return mainContextState.currentWorkflowStatus === workflowStatus.proposalsRegistrationStarted || mainContextState.workflowStatusChangedPastEvents[workflowEventArrayLegth - 1]?.lastCurrentStatus === "1" ? <Button key={index} title={elt.txt} color={elt.css} func={newFunc}/> : null;
        case solidityFunctionsList.voter.setVote:
          return mainContextState.currentWorkflowStatus === workflowStatus.votingSessionStarted || mainContextState.workflowStatusChangedPastEvents[workflowEventArrayLegth - 1]?.lastCurrentStatus === "3" ? <Button key={index} title={elt.txt} color={elt.css} func={newFunc}/> : null;
        default:
          return <Button key={index} title={elt.txt} color={elt.css} func={newFunc}/>;
      }
    } else {
